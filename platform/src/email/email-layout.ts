...
export interface EmailBody {
  ...
  /** Paragraphs after the button (expiry, "wasn't you", etc.). */
  outro: string[];
}
...
export function renderEmail(body: EmailBody): { html: string; text: string } {
  const p = (s: string) =>
    `<p style="margin:0 0 14px;font-size:15px;line-height:1.55;color:${INK}">${s}</p>`;
  ...
  <tr><td align="center" style="padding:28px 12px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:20px 28px 26px;font-family:Helvetica,Arial,sans-serif">
        ...
        ${body.intro.map(p).join('')}
        ${button}
        ${body.outro.map(p).join('')}
      </td></tr>
