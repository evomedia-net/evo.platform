...
cd templates/next    && npm run test:e2e        # browser tests; needs the app up on :4180
```

The browser tests (`e2e/`) drive the auth screens in Chromium against
standalone mode. They expect a migrated, seeded database and the app running
on port 4180 — `docker compose up -d`, `npm run db:migrate`, `npm run db:seed`,
`npm run dev` — and a browser installed once with `npx playwright install chromium`.
CI does all of that itself in the `e2e` job.
